...
const addRobotMarker = (position, heading = 0) => {
  ...
  el.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="40" height="40" style="transform: rotate(${heading}deg)">
      <circle cx="32" cy="32" r="30" fill="#007AFF" stroke="#0056B3" stroke-width="2"/>
      <polygon points="32,10 22,25 42,25" fill="#FFFFFF"/>
    </svg>
  `
...
const clearLayers = () => {
  ...
  layers.forEach(layer => {
    if (layer.id.includes('point-cloud') || layer.id.includes('robot-route') || layer.id.includes('robot-marker')) {
